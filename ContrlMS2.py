...
def generate_continuum_data(duration_sec=60, samples_per_sec=10):
    ...
    for i in range(total_samples):
        ...
        # Baseline: Idle state (2000mW system power, 2% Orchestrator CPU) [cite: 29]
        power = 2000 + random.uniform(-30, 30)
        cpu = 2 + random.uniform(-0.3, 0.3)
        
        # Add Spikes for Lifecycle Phases [cite: 73]
        # Instantiation (5-10s): Rapid compute aggregate creation [cite: 74]
        if 5 <= time <= 10:
            power += 800 + random.uniform(-50, 50)
            cpu += 12 + random.uniform(-1, 1)
        
        # Fusion (20-26s): Scaling by merging resources [cite: 77]
        elif 20 <= time <= 26:
            power += 1500 + random.uniform(-100, 100)
            cpu += 25 + random.uniform(-2, 2)
            
        # Migration (40-44s): Transferring execution context [cite: 80, 81]
        elif 40 <= time <= 44:
            power += 700 + random.uniform(-40, 40)
            cpu += 10 + random.uniform(-1, 1)
            
        # Dissolution (54-58s): Dismantling and entering deep sleep [cite: 78, 79]
        elif 54 <= time <= 58:
...
# 1. Generate and Save Data (The Digital Artifact) [cite: 98, 194]
...
# 3. Create Visualization of Control Cost [cite: 154, 155]
fig, ax1 = plt.subplots(figsize=(12, 6))

# System Power (Left Axis) [cite: 86]
...
# Control CPU Overhead (Right Axis - "Control Cost") [cite: 154]
ax2 = ax1.twinx()
...
# Annotate Malleable Phases [cite: 73]
